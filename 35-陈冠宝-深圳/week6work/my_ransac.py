...
def ransac(data, model, n, k, t, d, debug=False, return_all=False):
    ...
    while iterations < k:
        ...
        if (len(also_inliers) > d):
            ...
            bettermodel = model.fit(betterdata)#模型训练此处为最小二乘模型
            better_errs = model.get_error(betterdata, bettermodel)
            thiserr = np.mean(better_errs)  # 平均误差作为新的误差,只是更新本次训练得到的局内点对下次训练没有影响
            if thiserr < besterr:
                bestfit = bettermodel
                besterr = thiserr
...
def test():
    ...
    if 1:
        # 添加"局外点"100个离散分布
        n_outliers = 100#一共500个点，将其中的100个添加为局外点
        ...
        # B_noisy[outlier_idxs] = -100 * np.random.normal(size=(n_outliers, n_outputs))+50  # 加入噪声和局外点的Yi
        B_noisy[outlier_idxs] = B_noisy[outlier_idxs]+-1000 * np.random.random(size=(n_outliers, n_outputs))+500  # 加入噪声和局外点的Yi
    if 1:
        # 添加"局外点"100个高斯分布
        n_outliers = 100#一共500个点，将其中的100个添加为局外点
        all_idxs = np.arange(A_noisy.shape[0])  # 获取索引0-499
