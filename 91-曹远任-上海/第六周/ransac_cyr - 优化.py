...
def ransac(x, y):
    ...
    for ki in range(tryk):
        ...
        # 优化部分：在“内群”里看哪些点过于离群予以剔除 ###################
        intooutx = intoouty = np.array([])
        for i in range(insetx.shape[0]):
            if distance(k, b, insetx[i], insety[i]) > maxouterr:
                intooutx = np.append(intooutx, insetx[i])
                intoouty = np.append(intoouty, insety[i])
        # 删除内群对应元素
        for i in range(intooutx.shape[0]):
            insetx = np.delete(insetx, insetx == intooutx[i])
            insety = np.delete(insety, insety == intoouty[i])
        # 添加外群群对应元素
        outsetx = np.concatenate((outsetx, intooutx))
        outsety = np.concatenate((outsety, intoouty))
        # 优化部分至此
