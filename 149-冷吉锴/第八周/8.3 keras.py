"""
第一步
将训练数据和测试数据加载到内存中（第一次运行需要下载数据，会比较慢）
...
path = r'D:\Anaconda3\Anaconda\envs\CVandNLP\Lib\site-packages\tensorflow\python\keras\datasets\mnist.npz'
...
"""
1、train_images.shape打印结果表明，它是一个含有60000个元素的数组
数组中的元素是一个二维数组，二维数组的行和列都是28，是指一个数字图片的大小是28*28
2、train_labels打印结果表明，第一张手写数字图片的内容是数字5，第二张图片是数字0，以此类推
3、test_images.shape打印结果表明，用于检验训练效果的图片有10000张
...
network.add(layers.Dense(10, activation='softmax'))  # 输出层(0~9有10个类别)
# 优化器、损失函数、评价指标
...
test_loss, test_acc = network.evaluate(test_images, test_labels, verbose=1)  # 推理 verbose=1 输出每一轮迭代的进度条
print(test_loss)
