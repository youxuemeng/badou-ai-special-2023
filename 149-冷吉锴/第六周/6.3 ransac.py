...
class LinearLeastSquareModel:
    ...
    def fit(self, data):
        ...
        B = np.vstack([data[:, i] for i in self.output_columns]).T  # 第一列Yi-->行Yi
...
def test():
    ...
    perfect_fit = 60 * np.random.random(size=(n_inputs, n_outputs))  # 随机线性度，即随机生成一个斜率
    ...
    debug = False
    model = LinearLeastSquareModel(input_columns, output_columns, debug=debug)  # 实例化，用最小二乘法生成已知模型
