...
class NeuralNetWork:
    def __init__(self,inputnodes,hiddennodes,outputnodes,learningrate):
        ...
        这个函数用于生成正态分布（也称为高斯分布）的随机数。
        第一个参数 0.0 是正态分布的均值（mean）。
        第二个参数 pow(self.hnodes, -0.5) 是正态分布的标准差（standard deviation）。
        这里它使用 self.hnodes 的倒数的平方根，self.hnodes 可能代表隐藏层节点的数量。
        这种初始化方法有助于在网络训练初期保持权重的规模不太大也不太小，有利于优化过程。
