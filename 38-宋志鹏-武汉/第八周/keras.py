...
'''
添加两个Dense层（即全连接层）。
第一个Dense层有512个神经元，并使用ReLU（Rectified Linear Unit）激活函数。
它的input_shape设置为(28*28,)，
第二个Dense层是输出层，有10个神经元，每个神经元对应一个类别，使用softmax激活函数。
这使得网络输出一个10维的概率分布，每个维度代表一个类别的概率
'''
...
'''
编译模型
optimizer='rmsprop'指定了优化器（这里使用RMSprop）
loss='categorical_crossentropy'指定了损失函数（用于多类别分类问题）
metrics=['accuracy']意味着在训练和测试期间，模型会跟踪准确度
'''
