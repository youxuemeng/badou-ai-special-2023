...
def ransac(data,model,n,k,t,d,debug = False,return_all = False):
    ...
    :param debug:False不输出调试信息
    :param return_all:False:只返回最终结果
    :return:
    输出：
    bestfit: 最优拟合解（返回nil，如果未找到）

    iterations = 0 # 初始化迭代次数为0
    bestfit = None # 初始化最优拟合解为None，表示尚未找到最优解
    besterr = something really large # 初始化最优误差为一个非常大的值，后面会用实际的误差值更新它
    besterr = thiserr

    while iterations < k
    {
        maybeinliers = #从样本中随机选取n个，不一定全是局内点
        maybemodels =  #n个maybeinliers 拟合出来的可能符合要求的模型
        alsoinliers = emptyset  #满足误差要求的样本点，开始置空
        for（每一个不是maybeinliers的样本点）
        {
            if 满足maybemodel 即error <t
                将点加入alseinliers
        }
        if (alsoinliers样本点数目>d)
        {
            %有了较好的模型，测试模型符合度
            bettermedel = 利用所有的maybeinliers 和alseinliers重新生成更好的模型
            thiserr = 所有的maybeinliers 和alsoinliers样本点的误差度量
            if thiserr < beterr
            {
                bestfit = bettermodel
                besterr = thiserr
    ...
    while iterations < k :
        ...
        test_err = model.get_error(test_points,maybemodel) #计算误差：平方差最小
        ...
        iterations +=1
    if bestfit is None:
        raise ValueError("didn't meet fit accepttance criteria")
    if return_all:
...
class LinearLeastSquareModel:
    ...
    def fit(self,data):
        ...
        B = np.vstack([data[:,i] for i in self.output_columns] ).T #第二列Yi-->列Yi
        x,reside,rank,s = sl.lstsq(A,B) #residues:残差和
        return x #返回最小平方和向量
...
def test():
    ...
    if 1:
        ...
        outlier_idxs = all_idxs[:n_outliers]  # 100个0-500 随机局外点
    ...
    output_columns = [n_inputs + i for i in range(n_outputs)]  # 数组的最后一列y:1
    debug = False
    ...
    if 1:
        ...
        if 1:
            ...
            pylab.plot(A_noisy[ransac_data['inliers'],0], B_noisy[ransac_data['inliers'],0], 'bx',label="rangsac data")
        else:
            pylab.plot(A_noisy[non_outlier_idxs,0], B_noisy[non_oulier_idxs,0], 'k.', label='noisy data')
