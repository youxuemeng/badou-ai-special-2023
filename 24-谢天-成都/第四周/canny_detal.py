...
if __name__ == '__main__':
    p_path = 'C:\\Users\\32496\\Desktop\\lenna.png'
    img = plt.imread(p_path)
    if p_path[-4:] == '.png':  # .png图片在这里的存储格式是0到1的浮点数，所以要扩展到255再计算
        img = img * 255  # 还是浮点数类型
    ...
    # 4、双阈值检测，连接边缘。遍历所有一定是边的点,查看邻域是否存在有可能是边的点
    lower_boundary = img_tidu.mean() * 0.5
    high_boundary = lower_boundary * 3  
    zhan = []
    for i in range(1, img_yizhi.shape[0] - 1):  # 不考虑外圈
        for j in range(1, img_yizhi.shape[1] - 1):
            if img_yizhi[i, j] >= high_boundary:  # 取，边的点
                img_yizhi[i, j] = 255
                zhan.append([i, j])
